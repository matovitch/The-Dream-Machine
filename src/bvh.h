...
typedef struct
{
    ...
    union {
        unsigned int child;         /**< Children index (for inner nodes) */
        unsigned int tri_id;        /**< First triangle id (for leaf nodes) */
    } node_data;
    float box_max[3];               /**< Maximum dimension of the bounding box */
    unsigned short num_tris;        /**< Number of triangles (for a leaf node) */
    unsigned char axis;             /**< Dominant axis (0, 1 or 2) */
    unsigned char order;            /**< Traversal order (0 or 1) */
} bvh_node;
