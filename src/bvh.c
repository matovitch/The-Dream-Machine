...
void evaluate_partitions(const sah_bin* bins, unsigned int num_bins, float* sah_cost)
{
    ...
    sah_cost[num_bins - 1] = right_area * right_tris +
    ...
    for (int i = num_bins - 2; i > 0; i--) {
        ...
        sah_cost[i] = right_area * right_tris +
                            ...
                            bbox* boxes, unsigned int depth)
{
    ...
    float sah_cost[num_bins];
    evaluate_partitions(bins, num_bins, sah_cost);
    
    /* Find minimum cost */
    unsigned int min_cost = 1;
    for (unsigned int i = 2; i < num_bins; i++) {
        if (sah_cost[min_cost] > sah_cost[i])
            min_cost = i;
    }
